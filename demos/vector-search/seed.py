...
PRODUCTS = [
    (
        ...
        "electronics",
        "Premium over-ear headphones with ANC and 30-hour battery life",
        299.99,
    ),
    ...
    (
        ...
        "books",
        "300+ recipes for data science, web dev, and automation with Python",
        44.99,
    ),
...
async def _seed(count: int) -> None:
    ...
    # The vector-store sidecar indexes data asynchronously.
    # Wait a moment so ANN queries work immediately when the app starts.
    console.print("[dim]⏳ Waiting for vector-store to index embeddings...[/]")
    for i in track(range(10), description="[cyan]Indexing...[/]", console=console):
        await asyncio.sleep(1)
    console.print("[bold cyan]✓ Vector index is ready.[/]")
    console.print()
