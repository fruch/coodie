...
| Vector index | `CREATE CUSTOM INDEX … USING 'vector_index' WITH OPTIONS = {'similarity_function': 'COSINE'}` |
...
```bash
# From this directory:
make run    # starts ScyllaDB + vector-store sidecar, seeds 30 products, launches FastAPI app

# Then visit:
open http://127.0.0.1:8000
```

> **Infrastructure**: This demo uses its own `docker-compose.yml` (not the shared one) because it
> requires the `scylladb/vector-store` sidecar alongside ScyllaDB. Both containers share a Docker
> network so ScyllaDB can route ANN queries to the vector-store service.
