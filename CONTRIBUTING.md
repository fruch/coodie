...
| `/solve` | Merge the base branch into the PR branch to resolve conflicts. Unlike `/rebase`, this preserves commit history by using a merge strategy. If conflicts arise, Copilot CLI attempts to resolve them automatically. |
...
> **When to use `/rebase` vs `/squash` vs `/solve`:** Use `/rebase` when your
> branch is behind the base branch and you want to bring it up to date while
> preserving individual commits (rewrites history). Use `/squash` when the PR
> has many small or work-in-progress commits and you want a clean single-commit
> history before merging. Use `/rebase squash` to do both at once. Use `/solve`
> when you want to resolve merge conflicts without rewriting history — it merges
> the base branch into your PR branch (creating a merge commit).
