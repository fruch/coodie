...
| `GET` | `/ui/counters` | Counter list partial (auto-refreshes every 3s) |
...
- **Live auto-refresh** — counter table reloads every 3 seconds via HTMX polling
- **Increment / Decrement forms** — click to atomically update counters
- **Summary row** — shows total views and visitors across all URL/date combos
- **No-refresh updates** — HTMX swaps the counter table in place
