...
operations with an interactive **tag cloud** for filtering articles by tag:
...
| **Frozen collection** | `Annotated[set[str], Frozen()]` | `frozen<set<text>>` |
...
class FrozenTagSnapshot(Document):
    ...
    frozen_tags: Annotated[Optional[set[str]], Frozen()]  # → frozen<set<text>>
    note: str
